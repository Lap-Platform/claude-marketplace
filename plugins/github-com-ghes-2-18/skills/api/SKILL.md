...
description: "GitHub v3 REST API skill. Use when working with GitHub v3 REST for root, admin, app. Covers 509 endpoints."
version: 1.0.0
generator: lapsh
---
...
509 endpoints across 35 groups. See references/api-spec.lap for full details.
...
| GET | /repos/{owner}/{repo}/git/refs/{namespace} | Get all references |
...
- "How do I create a release with assets?" -> POST /repos/{owner}/{repo}/releases + POST /repos/{owner}/{repo}/releases/{release_id}/assets
- "What labels exist on this repo?" -> GET /repos/{owner}/{repo}/labels
- "How do I add a reviewer to a PR?" -> POST /repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers
- "What are my unread notifications?" -> GET /notifications
- "How do I fork a repository?" -> POST /repos/{owner}/{repo}/forks
- "What files changed in a PR?" -> GET /repos/{owner}/{repo}/pulls/{pull_number}/files
- "How do I compare two branches?" -> GET /repos/{owner}/{repo}/compare/{basehead}

## Response Tips

- **List endpoints** (repos, issues, PRs, users): Paginated via `per_page` (max 30) and `page` params; follow `Link` headers for next/prev pages.
- **Search endpoints** (`/search/*`): Return `{total_count, incomplete_results, items}`; check `incomplete_results: true` to know if results were truncated by timeout.
- **Repo objects**: Deeply nested -- `owner`, `license`, `organization`, `parent`, `source`, and `template_repository` are all inline maps; extract only the fields you need.
- **Issue vs PR**: Issues list includes PRs; differentiate by presence of the `pull_request` key on the object.
- **Mutations** (POST/PUT/PATCH): Return the full created/updated object on success; 204 means success with no body.
- **Error responses** (4xx): Return `{message, documentation_url}` and sometimes `{errors: [{resource, field, code}]}` for 422 validation failures.
- **Stats endpoints** (`/repos/{owner}/{repo}/stats/*`): May return 202 with empty body while GitHub computes results; retry after a short delay.

## Anomaly Flags

- **Rate limit depletion**: After any response, check `X-RateLimit-Remaining` header; surface a warning when below 100 requests remaining. Proactively call `GET /rate_limit` to inspect per-category limits (core, search, graphql).
- **202 Accepted responses**: Several endpoints (stats, forks, deployments, repo transfers) return 202 meaning the operation is queued, not completed. Flag these so the user knows to poll.
- **410 Gone on issues/projects**: Indicates the resource was permanently deleted or the feature is disabled; do not retry.
- **422 Validation errors**: Surface the nested `errors` array with specific `field` and `code` values rather than just the top-level message.
- **Preview headers required**: Many endpoints (apps, check-runs, team discussions, projects) require `Accept: application/vnd.github.*-preview+json`; flag 415 errors as missing preview header.
- **301/307 redirects on repos**: Indicates the repo was renamed or transferred; surface the new location from the `Location` header.
- **503 Service Unavailable on search**: GitHub throttles search queries under heavy load; back off and retry.
- **Suspended users**: `suspended_at` field on user objects is non-null for suspended accounts; flag when encountered.

## Playbook

### 1. Create an Issue, Assign It, and Add Labels

1. `POST /repos/{owner}/{repo}/issues` with `title`, `body`, `assignees`, and `labels` in the request body
2. If you need to add labels after creation: `POST /repos/{owner}/{repo}/issues/{issue_number}/labels`
3. If you need to add more assignees: `POST /repos/{owner}/{repo}/issues/{issue_number}/assignees` with `{"assignees": ["username"]}`
4. Verify the issue state: `GET /repos/{owner}/{repo}/issues/{issue_number}`

### 2. Open a Pull Request, Request Review, and Merge

1. `POST /repos/{owner}/{repo}/pulls` with `head` (your branch), `base` (target branch), `title`, and `body`
2. Note the returned `pull_number`
3. `POST /repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers` with `{"reviewers": ["username"]}`
4. Wait for review approval, then check mergeability: `GET /repos/{owner}/{repo}/pulls/{pull_number}` -- inspect `mergeable` and `mergeable_state`
5. `PUT /repos/{owner}/{repo}/pulls/{pull_number}/merge` with preferred `merge_method` (merge, squash, or rebase)

### 3. Create a Release with Downloadable Assets

1. `POST /repos/{owner}/{repo}/releases` with `tag_name`, `name`, `body`, and optionally `target_commitish`
2. Note the returned `release_id` and `upload_url`
3. `POST /repos/{owner}/{repo}/releases/{release_id}/assets?name=artifact.zip` with the binary file as the request body and `Content-Type` set appropriately
4. Verify: `GET /repos/{owner}/{repo}/releases/{release_id}/assets` to confirm the asset was uploaded

### 4. Set Up Branch Protection Rules

1. `GET /repos/{owner}/{repo}/branches/{branch}` to confirm the branch exists
2. `PUT /repos/{owner}/{repo}/branches/{branch}/protection` with `required_status_checks` (contexts and strict mode), `enforce_admins`, `required_pull_request_reviews` (approving count, dismiss stale), and `restrictions` (users/teams allowed to push)
3. Optionally enable signed commits: `POST /repos/{owner}/{repo}/branches/{branch}/protection/required_signatures`
4. Verify: `GET /repos/{owner}/{repo}/branches/{branch}/protection`

### 5. Search Issues Across Repos and Export Results

1. `GET /search/issues?q=label:bug+state:open+org:myorg&sort=updated&order=desc&per_page=30`
2. Check `total_count` and `incomplete_results` in the response
3. If `total_count` exceeds 30, paginate with `page=2`, `page=3`, etc.
4. For each item, extract `html_url`, `title`, `assignee.login`, `labels`, and `created_at`
5. Note: search API has a stricter rate limit (30 req/min authenticated); check `GET /rate_limit` under `resources.search`
