...
- "How do I send a document for signature?" -> POST /v2.1/accounts/{accountId}/envelopes
- "What envelopes are waiting for signatures?" -> GET /v2.1/accounts/{accountId}/envelopes (with `status=sent` or `from_to_status=sent`)
- "How do I check the status of a specific envelope?" -> GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}
- "Who are the recipients on this envelope and have they signed?" -> GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}/recipients
- "How do I download a completed document?" -> GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}/documents/{documentId}
- "How do I create a reusable template?" -> POST /v2.1/accounts/{accountId}/templates
- "How do I add a recipient to an existing envelope?" -> POST /v2.1/accounts/{accountId}/envelopes/{envelopeId}/recipients
- "How do I void or cancel an envelope?" -> PUT /v2.1/accounts/{accountId}/envelopes/{envelopeId} (set `status` to `voided` in envelope body)
- "How do I generate a signing URL for embedded signing?" -> POST /v2.1/accounts/{accountId}/envelopes/{envelopeId}/views/recipient
- "How do I send envelopes in bulk?" -> POST /v2.1/accounts/{accountId}/bulk_send_lists/{bulkSendListId}/send
- "What users exist in my account?" -> GET /v2.1/accounts/{accountId}/users
- "How do I check my billing plan and invoices?" -> GET /v2.1/accounts/{accountId}/billing_plan + GET /v2.1/accounts/{accountId}/billing_invoices
- "How do I get the audit trail for an envelope?" -> GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}/audit_events
- "How do I retrieve form field data from a completed envelope?" -> GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}/form_data
- "How do I set up a Connect webhook listener?" -> POST /v2.1/accounts/{accountId}/connect

## Response Tips

- **Envelopes listing**: Results are paginated via `start_position` and `count`; always check `resultSetSize` and `totalSetSize` to determine if more pages exist. Filter with `from_date`/`to_date` and `status` to avoid oversized responses.
- **Recipients**: Response nests recipients by type (`signers`, `carbonCopies`, `certifiedDeliveries`, etc.) -- iterate each array, not just `signers`. Check `status` field per recipient for completion state.
- **Documents**: GET on a document ID returns raw binary (PDF by default). Use `certificate=true` to include the signing certificate. The `documents` list endpoint returns metadata, not file content.
- **Templates**: Similar pagination to envelopes (`count`, `start_position`). The `include` param controls how much detail is returned; omit it for lightweight listing.
- **Billing**: Invoice and payment lists accept `from_date`/`to_date` as ISO 8601 strings. Past-due invoices have a dedicated endpoint separate from the main invoice list.
- **Views (embedded signing/sending)**: POST returns a `url` field with a short-lived token URL. These expire quickly (typically 5 minutes) -- generate just before redirecting the user.
- **Errors**: All endpoints return 400 for validation failures. Parse `errorCode` and `message` from the JSON body. Common codes: `INVALID_REQUEST_BODY`, `ENVELOPE_IS_NOT_COMPLETE`, `RECIPIENT_NOT_IN_SEQUENCE`.
- **Bulk send**: Batch status includes `batchSize`, `failed`, `queued`, `sent` counts. Poll the batch status endpoint until `queued` reaches 0.

## Anomaly Flags

- **Rate limiting**: DocuSign enforces per-account and per-integration-key rate limits. Surface HTTP 429 responses immediately and recommend exponential backoff. Monitor `X-RateLimit-Remaining` headers when present.
- **Envelope status mismatch**: Flag if an envelope's `status` is `declined` or `voided` when the user expects it to be `sent` or `completed` -- this indicates a recipient refused or the sender cancelled.
- **Past-due invoices**: Proactively call `/billing_invoices_past_due` when billing operations are in scope. Alert the user if any past-due invoices exist, as this can affect account capabilities.
- **Expired embedded URLs**: If a view URL generation happened more than 5 minutes ago, warn the user the URL has likely expired before they attempt to use it.
- **Connect failures**: When configuring or debugging webhooks, proactively check `/connect/failures` and surface any delivery failures with their error details.
- **Account deletion risk**: Flag any call to `DELETE /v2.1/accounts/{accountId}` with a strong warning -- this is irreversible and the `redact_user_data` option permanently destroys user data.
- **Chunked upload abandonment**: If a chunked upload was started but not committed (via `PUT` with `action=commit`), alert that the partial upload will eventually expire.
- **Lock conflicts**: When envelope or template operations fail, check the `/lock` endpoint -- another user or process may hold an edit lock.

## Playbook

### 1. Send a Document for Signature (End to End)

1. Create the envelope with document and recipients: `POST /v2.1/accounts/{accountId}/envelopes` with `envelopeDefinition` containing `documents`, `recipients` (with `signers` array), and `status: "sent"`.
2. If using embedded signing, generate a recipient view URL: `POST /v2.1/accounts/{accountId}/envelopes/{envelopeId}/views/recipient` with `recipientViewRequest` including `returnUrl`, `authenticationMethod`, `email`, `userName`, `recipientId`.
3. Monitor envelope status: `GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}` -- poll or use Connect webhooks for real-time updates.
4. Once `status` is `completed`, download the signed document: `GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}/documents/{documentId}`.
5. Optionally retrieve form data: `GET /v2.1/accounts/{accountId}/envelopes/{envelopeId}/form_data`.

### 2. Set Up Bulk Sending

1. Create a bulk send list with recipient data: `POST /v2.1/accounts/{accountId}/bulk_send_lists` with `bulkSendingList` containing recipient rows.
2. Test the bulk send before executing: `POST /v2.1/accounts/{accountId}/bulk_send_lists/{bulkSendListId}/test` with `bulkSendRequest` referencing an envelope or template ID.
3. Review test results for errors. Fix any validation issues in the list.
4. Execute the bulk send: `POST /v2.1/accounts/{accountId}/bulk_send_lists/{bulkSendListId}/send`.
5. Monitor batch progress: `GET /v2.1/accounts/{accountId}/bulk_send_batch/{bulkSendBatchId}` until `queued` count reaches 0.
6. Review individual envelope statuses: `GET /v2.1/accounts/{accountId}/bulk_send_batch/{bulkSendBatchId}/envelopes`.

### 3. Create and Use a Template

1. Create the template: `POST /v2.1/accounts/{accountId}/templates` with `envelopeTemplate` containing documents, recipient roles, and tabs.
2. Add recipient tabs (signature fields): `POST /v2.1/accounts/{accountId}/templates/{templateId}/recipients/{recipientId}/tabs`.
3. Preview the template: `POST /v2.1/accounts/{accountId}/templates/{templateId}/views/edit` to open the template editor.
4. Send an envelope from the template: `POST /v2.1/accounts/{accountId}/envelopes` with `templateId` in the `envelopeDefinition` and `templateRoles` mapping role names to actual recipients.
5. Optionally add to favorites: `PUT /v2.1/accounts/{accountId}/favorite_templates`.

### 4. Configure Connect Webhooks for Real-Time Notifications

1. Create a Connect configuration: `POST /v2.1/accounts/{accountId}/connect` with `connectCustomConfiguration` specifying `urlToPublishTo`, `envelopeEvents`, and `recipientEvents`.
2. Verify the configuration: `GET /v2.1/accounts/{accountId}/connect/{connectId}` to confirm settings.
3. Monitor delivery logs: `GET /v2.1/accounts/{accountId}/connect/logs` to verify events are flowing.
4. If deliveries fail, check failures: `GET /v2.1/accounts/{accountId}/connect/failures`.
5. Retry failed deliveries: `PUT /v2.1/accounts/{accountId}/connect/envelopes/retry_queue` with failure filter criteria.

### 5. Manage Account Users and Permissions

1. List current users: `GET /v2.1/accounts/{accountId}/users` with optional filters like `status`, `email`, or `group_id`.
2. Add a new user: `POST /v2.1/accounts/{accountId}/users` with `newUsersDefinition` containing email, name, and permission profile.
3. Review available permission profiles: `GET /v2.1/accounts/{accountId}/permission_profiles`.
4. Assign a user to a group: `PUT /v2.1/accounts/{accountId}/groups/{groupId}/users` with the user info.
5. Configure user settings: `PUT /v2.1/accounts/{accountId}/users/{userId}/settings` with `userSettingsInformation`.
