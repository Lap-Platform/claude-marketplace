...
- "How do I find repositories matching a keyword?" -> GET /search/repositories
- "What are the open pull requests on a repo?" -> GET /repos/{owner}/{repo}/pulls
- "How do I create an issue?" -> POST /repos/{owner}/{repo}/issues
- "Who are the collaborators on this repo?" -> GET /repos/{owner}/{repo}/collaborators
- "What workflows exist and what's their status?" -> GET /repos/{owner}/{repo}/actions/workflows
- "Is a specific workflow run passing or failing?" -> GET /repos/{owner}/{repo}/actions/runs/{run_id}
- "How do I merge a pull request?" -> PUT /repos/{owner}/{repo}/pulls/{pull_number}/merge
- "How do I check my rate limit?" -> GET /rate_limit
- "What branches exist and are they protected?" -> GET /repos/{owner}/{repo}/branches
- "How do I add a label to an issue?" -> POST /repos/{owner}/{repo}/issues/{issue_number}/labels
- "What secrets are configured for a repo's Actions?" -> GET /repos/{owner}/{repo}/actions/secrets
- "How do I create a release with a tag?" -> POST /repos/{owner}/{repo}/releases
- "How do I compare two branches or commits?" -> GET /repos/{owner}/{repo}/compare/{basehead}
- "How do I request reviewers on a PR?" -> POST /repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers
- "What are the deployment statuses for a deployment?" -> GET /repos/{owner}/{repo}/deployments/{deployment_id}/statuses

## Response Tips

- **List endpoints** (issues, PRs, repos, commits): Paginated via `per_page` (default 30, max 100) and `page` params; check `Link` header for next/last page URLs.
- **Search endpoints** (`/search/*`): Return `{total_count, incomplete_results, items}`; when `incomplete_results` is true, results are partial due to timeout.
- **Actions endpoints** (runs, artifacts, workflows): Wrap arrays in `{total_count, <items_key>}` objects; use `total_count` to know if you need to paginate.
- **Webhook deliveries**: Use cursor-based pagination (`cursor` param) instead of page-based; do not use `page`.
- **Delete/PUT operations**: Typically return 204 (no body) on success; a 404 means the resource was already gone or never existed.
- **User/org objects**: Deeply nested in most responses; the `owner`, `user`, `creator` maps share the same shape across all endpoints.
- **Stats endpoints** (`/repos/{owner}/{repo}/stats/*`): May return 202 (computing) with empty body on first request; retry after a short delay until you get 200.

## Anomaly Flags

- **Rate limit approaching**: Surface when `GET /rate_limit` shows `remaining` below 100 for `core` or below 5 for `search`; the `reset` field (Unix timestamp) tells when the budget refills.
- **Secondary rate limits**: A 403 with `retry-after` header or abuse detection message means you're sending too many requests too fast, even within your quota.
- **Incomplete search results**: When `incomplete_results: true` in any `/search/*` response, warn that results may be missing and suggest narrowing the query.
- **Stats 202 responses**: If `/stats/*` endpoints return 202, the data is being computed; alert the user that a retry is needed rather than treating it as empty.
- **422 Validation errors**: These carry a `message` and `errors` array with field-level detail; surface the specific field and code (e.g., `already_exists`, `missing_field`).
- **Repository redirects (301/307)**: A repo was renamed or transferred; surface the new URL from the `Location` header so the user can update references.
- **Deprecated legacy endpoints**: The `/teams/{team_id}/*` routes are legacy; prefer `/orgs/{org}/teams/{team_slug}/*` equivalents. Flag if legacy routes are used.
- **Check run/suite failures**: When `conclusion` is `failure`, `action_required`, or `timed_out`, proactively surface the failing check name and details URL.
...
2. Create the pull request: `POST /repos/{owner}/{repo}/pulls` with `head` (your branch), `base` (target branch), `title`, and `body`
3. Request reviewers: `POST /repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers` with `reviewers` array
4. Add labels: `POST /repos/{owner}/{repo}/issues/{pull_number}/labels` (PRs share the issues endpoint for labels)
5. Monitor checks: `GET /repos/{owner}/{repo}/commits/{ref}/check-runs` to confirm CI passes

### 2. Triage and Manage Issues

1. List open issues: `GET /repos/{owner}/{repo}/issues?state=open&sort=created&direction=desc`
2. Filter by label or assignee using `labels` and `assignee` query params
3. Assign a user: `POST /repos/{owner}/{repo}/issues/{issue_number}/assignees` with `assignees` array
4. Add labels: `POST /repos/{owner}/{repo}/issues/{issue_number}/labels`
5. Comment on the issue: `POST /repos/{owner}/{repo}/issues/{issue_number}/comments` with `body`
6. Close when resolved: `PATCH /repos/{owner}/{repo}/issues/{issue_number}` with `state: "closed"`

### 3. Set Up Branch Protection

1. Check current protection: `GET /repos/{owner}/{repo}/branches/{branch}/protection`
2. Apply protection rules: `PUT /repos/{owner}/{repo}/branches/{branch}/protection` with `required_status_checks` (CI contexts, strict mode), `enforce_admins`, `required_pull_request_reviews` (approving count, dismiss stale), and `restrictions` (users/teams allowed to push)
3. Enable required signatures: `POST /repos/{owner}/{repo}/branches/{branch}/protection/required_signatures`
4. Verify with: `GET /repos/{owner}/{repo}/branches/{branch}` and check `protected: true`

### 4. Create a Release with Assets

1. Tag the commit: `POST /repos/{owner}/{repo}/git/refs` with `ref: "refs/tags/v1.0.0"` and `sha`
2. Create the release: `POST /repos/{owner}/{repo}/releases` with `tag_name`, `name`, `body`, and optionally `prerelease: true`
3. Upload assets: `POST /repos/{owner}/{repo}/releases/{release_id}/assets?name=binary.zip` with the file as the request body
4. Verify: `GET /repos/{owner}/{repo}/releases/latest` to confirm the release and its assets are published

### 5. Monitor CI/CD Pipeline

1. List workflow runs: `GET /repos/{owner}/{repo}/actions/runs?branch=main&status=completed&per_page=5`
2. Inspect a specific run: `GET /repos/{owner}/{repo}/actions/runs/{run_id}` -- check `conclusion` field
3. Get job details: `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` -- each job has `steps` with individual step status
4. Download logs on failure: `GET /repos/{owner}/{repo}/actions/jobs/{job_id}/logs` (follows a 302 redirect to the log file)
5. Re-run if needed: `POST /repos/{owner}/{repo}/actions/runs/{run_id}/rerun`
