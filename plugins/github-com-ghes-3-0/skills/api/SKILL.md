...
description: "GitHub v3 REST API skill. Use when working with GitHub v3 REST for root, admin, app. Covers 674 endpoints."
version: 1.0.0
generator: lapsh
---
...
674 endpoints across 36 groups. See references/api-spec.lap for full details.
...
- "What repos does this user have?" -> GET /users/{username}/repos
- "How do I create a new issue?" -> POST /repos/{owner}/{repo}/issues
- "What are the open pull requests on this repo?" -> GET /repos/{owner}/{repo}/pulls
- "How do I merge a pull request?" -> PUT /repos/{owner}/{repo}/pulls/{pull_number}/merge
- "What workflows are available in this repo?" -> GET /repos/{owner}/{repo}/actions/workflows
- "Is this workflow run still going?" -> GET /repos/{owner}/{repo}/actions/runs/{run_id}
...
- "Am I being rate limited?" -> GET /rate_limit
...
- "Who are the members of this org?" -> GET /orgs/{org}/members
- "How do I fork a repo?" -> POST /repos/{owner}/{repo}/forks
- "What secrets are configured for this repo's actions?" -> GET /repos/{owner}/{repo}/actions/secrets

## Response Tips

- **List endpoints** (repos, issues, PRs, members): Paginated via `per_page` (default 30, max 100) and `page` params; check `Link` header for next/last page URLs.
- **Search endpoints** (`/search/*`): Returns `{total_count, incomplete_results, items}`; when `incomplete_results` is true, results are partial due to timeout -- retry or narrow the query.
- **Repo objects**: Deeply nested -- `owner`, `license`, `organization`, `parent`, `source`, and `template_repository` are all inline maps; extract what you need rather than processing the full tree.
- **Issue vs PR**: The `/issues` endpoints return both issues and PRs; check for the `pull_request` key to distinguish them.
- **Actions runs/jobs**: Status is one of `queued/in_progress/completed`; `conclusion` is only set when status is `completed`.
- **204 responses**: Successful deletes, membership checks, and star/follow operations return empty bodies -- treat 204 as success, 404 as "not found / not a member."
- **Webhook/hook objects**: The `config` map contains `url`, `content_type`, `secret`, and `insecure_ssl` -- `secret` is write-only and returned as `********` on reads.

## Anomaly Flags

- **Rate limit approaching**: After each response, check `X-RateLimit-Remaining`; surface a warning when below 100 requests remaining, and proactively call `GET /rate_limit` to show reset time.
- **Incomplete search results**: When `incomplete_results: true` is returned from any `/search/*` endpoint, alert the user that results may be missing.
- **202 Accepted responses**: Several endpoints (stats, deployments, repo transfers) return 202 meaning the request is queued, not complete -- surface this and suggest polling.
- **301/307 redirects**: Repo renames return 301; flag this so the user updates their references to the new owner/repo path.
- **403 with abuse detection**: GitHub returns 403 with a `Retry-After` header for secondary rate limits; surface the wait time instead of retrying immediately.
- **Deprecated token endpoints**: The `/applications/{client_id}/tokens/{access_token}` GET/POST/DELETE endpoints are legacy -- flag and suggest `/applications/{client_id}/token` (singular) instead.
- **422 Validation errors**: These contain a detailed `errors` array with `field`, `code`, and `message` -- surface the specific field that failed, not just the status code.
- **Preview headers required**: Admin endpoints (`/admin/*`) require `accept: application/vnd.github.superpro-preview+json` -- flag if the header is missing before making the call.

## Playbook

### 1. Create a PR from a Feature Branch

1. List branches to confirm your feature branch exists: `GET /repos/{owner}/{repo}/branches`
2. Create the pull request: `POST /repos/{owner}/{repo}/pulls` with `head` (feature branch), `base` (e.g. main), `title`, and `body`
3. Request reviewers: `POST /repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers` with `reviewers` array
4. Monitor review status: `GET /repos/{owner}/{repo}/pulls/{pull_number}/reviews`
5. Merge when approved: `PUT /repos/{owner}/{repo}/pulls/{pull_number}/merge` with preferred `merge_method` (merge/squash/rebase)

### 2. Set Up Branch Protection

1. Get current branch info: `GET /repos/{owner}/{repo}/branches/{branch}`
2. Apply protection rules: `PUT /repos/{owner}/{repo}/branches/{branch}/protection` with `required_status_checks` (contexts and strict mode), `enforce_admins`, `required_pull_request_reviews` (approving count, dismiss stale), and `restrictions` (users/teams)
3. Optionally enable required signatures: `POST /repos/{owner}/{repo}/branches/{branch}/protection/required_signatures`
4. Verify protection is active: `GET /repos/{owner}/{repo}/branches/{branch}/protection`

### 3. Monitor a CI/CD Workflow Run

1. List workflows to find the workflow ID: `GET /repos/{owner}/{repo}/actions/workflows`
2. Trigger a run if needed: `POST /repos/{owner}/{repo}/actions/workflows/{workflow_id}/dispatches` with `ref`
3. List recent runs: `GET /repos/{owner}/{repo}/actions/workflows/{workflow_id}/runs`
4. Check run details: `GET /repos/{owner}/{repo}/actions/runs/{run_id}` -- look at `status` and `conclusion`
5. If failed, inspect jobs: `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` and download logs: `GET /repos/{owner}/{repo}/actions/jobs/{job_id}/logs`
6. Re-run if needed: `POST /repos/{owner}/{repo}/actions/runs/{run_id}/rerun`

### 4. Issue Triage Workflow

1. List open issues: `GET /repos/{owner}/{repo}/issues?state=open&sort=created&direction=asc`
2. Add labels: `POST /repos/{owner}/{repo}/issues/{issue_number}/labels`
3. Assign team members: `POST /repos/{owner}/{repo}/issues/{issue_number}/assignees` with `assignees` array
4. Add to a milestone: `PATCH /repos/{owner}/{repo}/issues/{issue_number}` with `milestone` ID
5. Comment with triage notes: `POST /repos/{owner}/{repo}/issues/{issue_number}/comments` with `body`

### 5. Create and Publish a Release

1. List existing tags: `GET /repos/{owner}/{repo}/tags`
2. Create the release: `POST /repos/{owner}/{repo}/releases` with `tag_name`, `target_commitish`, `name`, `body`, and `prerelease` flag
3. Upload assets: `POST /repos/{owner}/{repo}/releases/{release_id}/assets?name=filename` with the binary body
4. Verify: `GET /repos/{owner}/{repo}/releases/latest` to confirm it is published
5. If corrections are needed: `PATCH /repos/{owner}/{repo}/releases/{release_id}` to update name, body, or promote from prerelease
