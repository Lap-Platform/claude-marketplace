...
description: "GitHub v3 REST API skill. Use when working with GitHub v3 REST for root, admin, app. Covers 766 endpoints."
version: 1.0.0
generator: lapsh
---
...
766 endpoints across 36 groups. See references/api-spec.lap for full details.
...
- "Show me the open pull requests for a repo" -> GET /repos/{owner}/{repo}/pulls
- "Create a new issue in a repository" -> POST /repos/{owner}/{repo}/issues
- "What are my current rate limits?" -> GET /rate_limit
- "Search for repositories matching a keyword" -> GET /search/repositories
- "Who are the collaborators on this repo?" -> GET /repos/{owner}/{repo}/collaborators
- "Merge this pull request" -> PUT /repos/{owner}/{repo}/pulls/{pull_number}/merge
- "List all workflow runs for a repo" -> GET /repos/{owner}/{repo}/actions/runs
- "Get the contents of a file in a repo" -> GET /repos/{owner}/{repo}/contents/{path}
- "Add a label to an issue" -> POST /repos/{owner}/{repo}/issues/{issue_number}/labels
- "Create a new release with a tag" -> POST /repos/{owner}/{repo}/releases
- "What branches does this repo have?" -> GET /repos/{owner}/{repo}/branches
- "Review a pull request" -> POST /repos/{owner}/{repo}/pulls/{pull_number}/reviews
- "List all secrets for a repo's Actions" -> GET /repos/{owner}/{repo}/actions/secrets
- "Compare two branches or commits" -> GET /repos/{owner}/{repo}/compare/{basehead}

## Response Tips

- **List endpoints** (repos, issues, PRs, commits): Paginated via `page` and `per_page` (default 30, max 100). Check `Link` header for next/prev pages.
- **Search endpoints** (`/search/*`): Return `{total_count, incomplete_results, items}`. Watch `incomplete_results: true` -- results may be partial due to timeouts.
- **Actions endpoints** (runs, artifacts, workflows): Wrapped in `{total_count, <items_array>}` pattern. Filter runs by `status`, `branch`, or `actor`.
- **Mutation endpoints** (POST/PUT/PATCH): Return 201 (created) or 200 (updated) with the full object. 204 means success with no body.
- **Error responses**: 422 = validation failed (check response body for field-level errors). 409 = conflict (merge conflicts, repo not empty). 403 = permission denied or secondary rate limit hit.
- **User/org objects**: Deeply nested -- `owner`, `creator`, `user` maps appear inside nearly every resource. Use `login` as the stable identifier, not `name`.

## Anomaly Flags

- **Rate limit approaching**: Surface when `GET /rate_limit` shows `remaining` below 100 for `core` or below 5 for `search`. The `reset` field is a Unix timestamp for when it refills.
- **Secondary rate limits**: A 403 with `Retry-After` header or "abuse detection" message means you are sending too many requests too fast. Back off immediately.
- **Incomplete search results**: When `incomplete_results: true` in any `/search/*` response, warn that results were truncated due to server timeout.
- **202 Accepted responses**: Stats endpoints (`/stats/*`) and some Actions endpoints return 202 when data is being computed. Retry after a short delay.
- **Merge conflicts**: 409 on merge attempts means the PR has conflicts. Surface the `message` field.
- **Repository redirects**: 301 on repo endpoints means the repo was renamed or transferred. Follow the redirect URL in the response.
- **Deprecated authorization endpoints**: The `/authorizations` group uses legacy OAuth tokens. Flag usage and suggest migrating to GitHub Apps or fine-grained PATs.
- **Suspended installations/users**: `suspended_at` being non-null on installations or users indicates suspended state. Surface proactively.

## Playbook

### 1. Create a Pull Request and Request Reviewers

1. Create or push a branch with your changes
2. `POST /repos/{owner}/{repo}/pulls` with `head` (your branch), `base` (target branch), `title`, and `body`
3. Note the `number` from the response
4. `POST /repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers` with `reviewers` (array of usernames)
5. Optionally add labels: `POST /repos/{owner}/{repo}/issues/{pull_number}/labels`

### 2. Set Up Branch Protection

1. `GET /repos/{owner}/{repo}/branches/{branch}` to confirm the branch exists
2. `PUT /repos/{owner}/{repo}/branches/{branch}/protection` with `required_status_checks` (set `strict: true` and `contexts`), `enforce_admins`, `required_pull_request_reviews` (set `required_approving_review_count`), and `restrictions`
...
### 3. Triage and Manage Issues

1. `GET /repos/{owner}/{repo}/issues?state=open&sort=created&direction=desc` to list recent issues
2. For each issue, assign it: `POST /repos/{owner}/{repo}/issues/{issue_number}/assignees` with `assignees`
3. Add labels: `POST /repos/{owner}/{repo}/issues/{issue_number}/labels`
4. Add a comment: `POST /repos/{owner}/{repo}/issues/{issue_number}/comments` with `body`
5. Close when resolved: `PATCH /repos/{owner}/{repo}/issues/{issue_number}` with `state: "closed"` and `state_reason: "completed"`

### 4. Monitor and Rerun CI Workflows

1. `GET /repos/{owner}/{repo}/actions/runs?status=failure` to find failed runs
2. `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` to see which jobs failed
3. `GET /repos/{owner}/{repo}/actions/jobs/{job_id}/logs` (follows 302 redirect) to read logs
4. Fix the issue, then `POST /repos/{owner}/{repo}/actions/runs/{run_id}/rerun` to retry
5. Check rate limits periodically: `GET /rate_limit` to avoid hitting secondary limits during automation

### 5. Create a Release with Assets

1. Ensure a tag exists or let the API create one
2. `POST /repos/{owner}/{repo}/releases` with `tag_name`, `name`, `body`, and `draft: false`
3. Note the `upload_url` from the response (it is a hypermedia template)
4. `POST /repos/{owner}/{repo}/releases/{release_id}/assets?name=binary.zip` with the binary content and appropriate `Content-Type`
5. Verify: `GET /repos/{owner}/{repo}/releases/latest` to confirm it is published
